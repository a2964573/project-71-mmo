...
int GameServer::broadcast(char* data, int size, int sender_fd)
{   
...
void GameServer::handleMovePacket(int client_socket, char* data, int size)
{
...
int GameServer::workerLogic(int worker_id)
{               
	while(true) {
		Job crnt_job;
		{   
			std::unique_lock<std::mutex> lock(que_mtx);
			cv.wait(lock, [this] { return !job_que.empty(); });
...
int GameServer::procedure(int epoll_fd, int client_socket)
{
	int  read_bytes   = 0;
	char buffer[1024] = {0,};
...
bool GameServer::Start()
{
	...
	for(i = 0; i< WORKER_COUNT; i++) {
		workers.emplace_back(&GameServer::workerLogic, this, i + 1);
		workers.back().detach();
	}
